...
class _ScannedParticipantsScreenState extends State<ScannedParticipantsScreen>
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return const Center(child: MyAppLoader());
    }
          ...
          children: [
                ...
                Row(
                  children: [
                    IconButton(
                      ...
                      onPressed: () => AppNavigator.fade(context, MainShell()),
            ...
            Expanded(
              child: filteredList.isEmpty
                  ? const Center(child: Text('No matching participants found.'))
                  : ListView.builder(
                      padding: const EdgeInsets.symmetric(horizontal: 12),
                      itemCount: filteredList.length + 1,
                      itemBuilder: (context, index) {
                        if (index == filteredList.length) {
                          // آخر عنصر: المسافة الفاضية تحت
                          return const SizedBox(
                            height: 120,
                          ); // المسافة اللي انت عايزها
                        }
